...
import { Eye, EyeOff, Loader2, GitFork, AlertTriangle, CheckCircle2, Wifi, WifiOff } from "lucide-react";
...
function OAuthButton({ icon, label, disabled = true }: { icon: React.ReactNode; label: string; disabled?: boolean }) {
  return (
    <button
      type="button"
      disabled={disabled}
      className="flex items-center justify-center gap-2.5 w-full px-4 py-2.5 rounded-xl text-sm font-medium border transition-colors disabled:opacity-40 disabled:cursor-not-allowed hover:bg-white/5"
      style={{ borderColor: "var(--pc-border)", color: "var(--pc-foreground)" }}
      title={disabled ? "OAuth not configured — contact your administrator" : undefined}
    >
      {icon}
      {label}
    </button>
...
function SignInForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button
        type="submit"
        disabled={loading || !apiKey.trim() || attempts >= 5}
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <OAuthButton
          label="Microsoft"
          icon={<svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M11.4 24H0V12.6h11.4V24zM24 24H12.6V12.6H24V24zM11.4 11.4H0V0h11.4v11.4zM24 11.4H12.6V0H24v11.4z" fill="#00A4EF"/></svg>}
        />
        <OAuthButton
          label="Enterprise SSO"
          icon={<svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>}
        />
      </div>
...
export default function LoginPage() {
  return (
    <div className="min-h-screen flex" style={{ background: "var(--pc-bg)" }}>
      ...
      {/* ── Left panel — form ─────────────────────────────────────── */}
      <div className="flex flex-col justify-center w-full lg:w-[480px] xl:w-[520px] flex-shrink-0 px-8 py-12 lg:px-14">
